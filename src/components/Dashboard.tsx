...
export const MetricTooltip = ({
  title,
  tooltip,
  titleClassName = "text-sm text-primary/60 font-medium",
  align = "center",
  className = "mb-1",
}: {
  title: string;
...
export function Dashboard({ building, onBack }: DashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<
    ...
    | "ai"
  >("loss");
  const [activeContractorCategory, setActiveContractorCategory] =
    useState("All");
  ...
  const [trackingPeriod, setTrackingPeriod] = useState<"1Y" | "YTD" | "ALL">(
    "1Y",
  );
  ...
  useEffect(() => {
    if (refinedFinancials) {
      localStorage.setItem(
        `refined_${building.id}`,
        JSON.stringify(refinedFinancials),
      );
    }
  ...
  const buildingSize = parseFloat(
    building.gross_floor_area_buildings_sq_ft || "0",
  );
    ...
    try {
      if (isInPortfolio && portfolioDocId) {
        ...
        await setDoc(newDocRef, {
          ...
          ghgEmissions: parseFloat(
            building.total_ghg_emissions_metric_tons_co2e || "0",
          ),
  ...
  const seasonalFactors = [
    1.1, 1.0, 0.9, 0.8, 0.9, 1.2, 1.3, 1.3, 1.1, 0.9, 1.0, 1.1,
  ];
  ...
  const actionsData = [
    ...
    {
      savings: financials.savingsPotential * 0.2,
      cost: financials.savingsPotential * 0.2 * 3.5,
      paybackMonths: 42,
      roi: 28,
    },
    {
      savings: financials.savingsPotential * 0.1,
      cost: financials.savingsPotential * 0.1 * 5.0,
      paybackMonths: 60,
      roi: 20,
    },
    {
    ...
    {
      savings: financials.savingsPotential * 0.2,
      cost: financials.savingsPotential * 0.2 * 7.0,
      paybackMonths: 84,
      roi: 14,
    },
    {
  ...
  const selectedSavings = selectedActions.reduce(
    (sum, idx) => sum + actionsData[idx].savings,
    0,
  );
  const selectedCost = selectedActions.reduce(
    (sum, idx) => sum + actionsData[idx].cost,
    0,
  );
  const selectedTenYearSavings = selectedSavings * 10 - selectedCost;
  const selectedROI =
    selectedCost > 0 ? (selectedSavings / selectedCost) * 100 : 0;
  ...
  const allTimeChartData = useMemo(() => {
    ...
    for (let i = -24; i <= 12; i++) {
      ...
      data.push({
        ...
        projected: isFuture
          ? Math.round(
              baseline - dynamicMonthlySavings * seasonalFactors[monthIdx],
            )
          : i === 0
            ? Math.round(baseline)
            : null,
  ...
  const filteredChartData = useMemo(() => {
    ...
    if (trackingPeriod === "YTD") {
      return allTimeChartData.filter(
      ...
      return allTimeChartData.filter(
        (d) => d.date >= oneYearAgo && d.date <= oneYearFuture,
      );
    }
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      const csvData = filteredChartData.map(
        (row) =>
          `${row.month},${row.actual || ""},${row.projected || ""},${row.baseline}`,
      );
      ...
      link.setAttribute(
        "download",
        `energy_cost_trend_${trackingPeriod.toLowerCase()}.csv`,
      );
  ...
  const pieData = [
    {
      name: "Electricity",
      value: financials.electricityCost,
      color: "#3b82f6",
    },
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="flex bg-black/5 p-1 rounded-full overflow-x-auto hide-scrollbar max-w-[calc(100vw-2rem)] sm:max-w-none">
                ...
                <button
                  ...
                  className={cn(
                    "px-4 py-1.5 text-sm font-medium rounded-full transition-all whitespace-nowrap flex items-center gap-1.5",
          ...
          <div className="relative w-full">
            {activeTab !== "loss" && (
              <div className="absolute inset-0 z-50 flex items-start justify-center pt-32 pointer-events-auto">
                <div className="bg-white p-8 rounded-2xl shadow-xl max-w-sm text-center flex flex-col items-center border border-black/5 mx-4 relative overflow-hidden">
                  ...
                  <h3 className="text-2xl font-semibold text-primary tracking-tight mb-2 relative z-10">
                    Premium Feature Locked
                  </h3>
                  <p className="text-primary/60 mb-8 relative z-10">
                    Join the waitlist for exclusive early access to diagnostics,
                    personalized action plans, and contractor bidding.
                  </p>
                  <button
                    onClick={() => navigate("/")}
                    className="bg-primary hover:bg-black text-white px-8 py-3.5 rounded-full font-medium transition-colors w-full relative z-10 shadow-sm"
                  >
                    Join the Waitlist
                  </button>
                </div>
            ...
            <div
              className={cn(
                ...
                activeTab !== "loss" &&
                  "filter blur-md opacity-40 pointer-events-none select-none",
              )}
            >
              {activeTab === "map" && (
                <motion.div
                  ...
                  <div className="bg-white rounded-3xl p-8 shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-black/5 relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 mb-6 relative z-10">
                      ...
                      <div>
                        ...
                        <p className="text-primary/60">
                          A synthesized overview of your building's energy
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
                      <div className="lg:col-span-2 space-y-8">
                        <div className="space-y-4">
                          ...
                          <p className="text-primary/80 leading-relaxed">
                            Your building is currently losing an estimated{" "}
                            <span className="font-semibold text-rose-600">
                              {formatValue(financials.estimatedWastedEnergy)}
                            </span>{" "}
                            annually due to inefficiencies. With an ENERGY STAR
                            score of{" "}
                            <span className="font-semibold">
                              {building.energy_star_score || "N/A"}
                            </span>
                            , it performs in the bottom half of similar
                            properties in Chicago. The primary culprits are
                            likely aging HVAC systems, poor envelope insulation,
                        ...
                        <div className="space-y-4">
                          ...
                          <p className="text-primary/80 leading-relaxed">
                            By implementing the recommended action plan—starting
                            with LED retrofits and HVAC optimization—you can
                            eliminate up to{" "}
                            <span className="font-semibold text-blue-600">
                              {financials.savingsPotentialPercentage.toFixed(1)}
                              %
                            </span>{" "}
                            of your energy waste. These upgrades are not just
                            expenses; they are investments that will increase
                            your Net Operating Income (NOI) by an estimated{" "}
                            <span className="font-semibold text-emerald-600">
                              {formatValue(financials.increasedNOI)}
                            </span>{" "}
                            per year.
                          </p>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <p className="text-primary/80 leading-relaxed">
                            Beyond immediate utility savings, these improvements
                            will boost your building's overall asset value by
                            approximately{" "}
                            <span className="font-semibold text-primary">
                              {formatValue(financials.increasedBuildingValue)}
                            </span>
                            . Furthermore, you will reduce your carbon footprint
                            by{" "}
                            <span className="font-semibold">
                              {building.total_ghg_emissions_metric_tons_co2e
                                ? (
                                    Number(
                                      building.total_ghg_emissions_metric_tons_co2e,
                                    ) * 0.2
                            ...
                            annually, aligning with Chicago's sustainability
                            goals and avoiding potential future emissions
                            penalties.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="bg-bg rounded-2xl p-6 space-y-6">
                        <h4 className="font-semibold text-primary border-b border-black/5 pb-4">
                          Key Takeaways
                        </h4>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <p className="text-2xl font-bold text-emerald-600">
                              {formatValue(financials.savingsPotential)}{" "}
                          ...
                          <div>
                            ...
                            <p className="text-2xl font-bold text-amber-600">
                              {formatValue(financials.estimatedTaxDeduction)}
                            </p>
                            <p className="text-xs text-amber-700/70 mt-1">
                              179D + Utility Rebates
                            </p>
                          </div>
                        </div>
                  ...
                  {refinedFinancials && (
                    <div className="md:col-span-3 bg-gradient-to-r from-emerald-50 to-green-50/30 border border-emerald-100 rounded-2xl p-4 flex items-start sm:items-center justify-between gap-4 shadow-sm relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-3 relative z-10">
                        ...
                        <div>
                          <h4 className="font-semibold text-emerald-800 tracking-tight">
                            Impact Analysis Refined
                          </h4>
                          <p className="text-sm text-emerald-700/80 mt-0.5">
                            Financial projections are now based on actual
                            utility bill data.
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-[0_2px_10px_rgba(0,0,0,0.04)] relative flex flex-col lg:flex-row gap-8">
                      <div className="flex-1 relative z-10">
                        ...
                        <h3 className="text-5xl font-semibold text-primary tracking-tight mb-2">
                          {formatValue(financials.monthlyLoss)}{" "}
                          <span className="text-2xl text-primary/60 font-medium">
                            / mo
                          </span>
                        </h3>
                        <p className="text-xl text-primary/70 mb-8">
                          This building is losing an estimated{" "}
                          <span className="text-rose-600 font-medium">
                            {formatValue(financials.estimatedWastedEnergy)}
                          </span>{" "}
                          annually to energy inefficiency.
                        </p>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                          <div className="bg-bg rounded-2xl p-4">
                            <MetricTooltip
                              title="Daily Bleed"
                              tooltip="The estimated amount of money lost every single day due to building inefficiencies and wasted energy."
                              align="left"
                            />
                            <p className="text-2xl font-semibold text-primary">
                              {formatValue(financials.dailyLoss)}
                            </p>
                          </div>
                          <div className="bg-bg rounded-2xl p-4">
                            <MetricTooltip
                              title="Savings Potential"
                              tooltip="The total estimated annual savings achievable by implementing recommended energy efficiency upgrades."
                            />
                            <p className="text-2xl font-semibold text-primary">
                              {formatValue(financials.savingsPotential)}
                            </p>
                          ...
                          <div className="bg-bg rounded-2xl p-4 col-span-2 md:col-span-1">
                            <MetricTooltip
                              ...
                              tooltip="The percentage of your total energy costs that could be eliminated through efficiency improvements."
                              align="right"
                            />
                            <p className="text-2xl font-semibold text-primary">
                              {financials.savingsPotentialPercentage.toFixed(1)}
                        ...
                        <div className="mt-8 pt-8 border-t border-black/5">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div className="bg-emerald-50 rounded-2xl p-4 border border-emerald-100">
                              <MetricTooltip
                                title="Increased NOI"
                                tooltip="Net Operating Income increase resulting directly from reduced annual operating expenses (energy savings)."
                                align="left"
                              />
                            ...
                            <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100">
                              <MetricTooltip
                                title="Building Value Increase"
                                tooltip="The estimated increase in the building's asset value, calculated by dividing the Increased NOI by the local capitalization rate."
                                align="right"
                              />
                              ...
                              <p className="text-xs text-blue-700/70 mt-1">
                                Based on {financials.capRate * 100}% cap rate
                              </p>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-8 border-t border-black/5">
                        <div className="bg-bg rounded-2xl p-4">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <MetricTooltip
                              ...
                              tooltip="Annual energy cost per square foot of building space."
                              titleClassName="text-xs text-primary/60 font-medium"
                              align="left"
                              className="mb-0"
                            />
                          </div>
                          <p className="text-xl font-semibold text-primary tracking-tight">
                            {formatValue(financials.costPerSqFt)}{" "}
                        ...
                        <div className="bg-bg rounded-2xl p-4">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <MetricTooltip
                              title="Site EUI"
                              tooltip="Site Energy Use Intensity (EUI) measures the amount of energy consumed per square foot per year."
                              titleClassName="text-xs text-primary/60 font-medium"
                              align="center"
                              className="mb-0"
                            />
                          ...
                          <p className="text-xl font-semibold text-primary tracking-tight">
                            {building.site_eui_kbtu_sq_ft || "N/A"}{" "}
                            <span className="text-xs font-normal text-primary/50">
                              kBtu/sqft
                            </span>
                          </p>
                        ...
                        <div className="bg-bg rounded-2xl p-4">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <MetricTooltip
                              title="GHG Emissions"
                              tooltip="Total Greenhouse Gas Emissions in Metric Tons of CO2 equivalent per year."
                              titleClassName="text-xs text-primary/60 font-medium"
                              align="center"
                              className="mb-0"
                            />
                        ...
                        <div className="bg-bg rounded-2xl p-4 border border-primary/20">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <MetricTooltip
                              ...
                              tooltip="Estimated federal tax deduction available for energy-efficient commercial building property."
                              titleClassName="text-xs text-primary/60 font-medium"
                              align="right"
                              className="mb-0"
                            />
                          ...
                          <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-6">
                            <div className="max-w-xl">
                              ...
                              <p className="text-white/70 text-sm leading-relaxed">
                                Civic Energy funds your retrofits with zero
                                upfront capital. Upgrade costs are covered
                                entirely through guaranteed energy savings,
                                creating immediate positive cash flow and asset
                                value growth.
                              </p>
                            </div>
                            <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
                              <button
                                onClick={() =>
                                  navigate(
                                    auth.currentUser ? "/pricing" : "/signup",
                  ...
                  <div className="space-y-6">
                    <div className="bg-white rounded-3xl p-6 shadow-[0_2px_10px_rgba(0,0,0,0.04)]">
                      <div className="flex items-center justify-between mb-4">
                        <MetricTooltip
                          title="Total Energy Spend"
                          tooltip="The estimated total annual cost of electricity and natural gas for this building."
                        />
                        <DollarSign className="w-5 h-5 text-primary/40" />
                      </div>
                      <p className="text-3xl font-semibold text-primary tracking-tight">
                        {formatValue(financials.totalAnnualCost)}
                      </p>
                      ...
                      <div className="mt-4 space-y-3">
                        <div className="flex justify-between items-center text-sm">
                          ...
                          <span className="font-medium text-primary">
                            {formatValue(financials.electricityCost)}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between items-center text-sm pt-2">
                          ...
                          <span className="font-medium text-primary">
                            {formatValue(financials.gasCost)}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-black/5 space-y-2">
                        <div className="flex justify-between items-center text-xs">
                          ...
                          <span className="font-medium text-blue-600">
                            {formatValue(financials.estimatedCoolingCost)}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-primary/60">
                          ...
                          <span className="font-medium text-orange-600">
                            {formatValue(financials.estimatedHeatingCost)}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl p-6 text-primary shadow-sm border border-black/5 relative">
                      ...
                      <MetricTooltip
                        ...
                        tooltip="Your building's ENERGY STAR score compared to similar properties. A score of 50 is the national median."
                        align="left"
                      />
                        ...
                        <div className="mt-4 relative z-10">
                          ...
                          <div className="w-full bg-black/5 rounded-full h-2 relative">
                            ...
                            <div
                              ...
                              style={{
                                left: `calc(${building.energy_star_score}% - 8px)`,
                              }}
                            ></div>
                          </div>
                          <p className="text-xs text-rose-500 mt-3 font-medium">
                            Your building is in the bottom{" "}
                            {financials.peerPercentile}% of similar Chicago
                            buildings.
                          </p>
                        </div>
                      )}
                    ...
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
                      <div className="max-w-3xl">
                        ...
                        <p className="text-lg text-primary/70">
                          Based on your building's age (
                          {building.year_built || "Unknown"}), size (
                          {Number(
                            building.gross_floor_area_buildings_sq_ft,
                          ).toLocaleString()}{" "}
                          sq ft), and energy profile compared to similar{" "}
                          {building.primary_property_type} properties in
                          Chicago, we've identified the likely sources of waste.
                        </p>
                        <div className="mt-4 p-4 bg-blue-50/50 rounded-2xl text-sm text-primary/80 flex items-start gap-3">
                          ...
                          <p>
                            ...
                            Local heating degree days (HDD) and cooling degree
                            days (CDD) heavily drive these costs. Your
                            building's envelope and HVAC systems are struggling
                            to maintain efficiency during extreme seasonal
                            shifts.
                          </p>
                        </div>
                      </div>
                      <div className="shrink-0 bg-bg p-4 rounded-2xl text-center">
                        ...
                        <div className="flex items-center gap-1 justify-center text-primary mb-2">
                          <div
                            className={cn("w-2 h-4 rounded-sm", "bg-primary")}
                          ></div>
                          <div
                            className={cn("w-2 h-4 rounded-sm", "bg-primary")}
                          ></div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
                    <div>
                      ...
                      <p className="text-sm text-primary/60 mt-1">
                        Select actions to build your custom plan
                      </p>
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="relative">
                      ...
                      <ActionCard
                        ...
                        incentives={[
                          "ComEd Standard Lighting Rebate",
                          "179D Tax Deduction Eligible",
                        ]}
                    ...
                    <ActionCard
                      ...
                      incentives={[
                        "ComEd Custom HVAC Rebate",
                        "IRA Commercial Credit",
                      ]}
                    ...
                    <ActionCard
                      ...
                      incentives={[
                        "ComEd Custom Rebate",
                        "Retro-Commissioning (RCx) Incentive",
                      ]}
                  ...
                  <div className="bg-white rounded-3xl p-8 border border-black/5 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                      <div>
                        ...
                        <p className="text-primary/60">
                          Pre-vetted contractors ready for your action plan.
                        </p>
                      </div>
                      <button
                        onClick={() => {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {[
                        ...
                        .filter(
                          (c) =>
                            activeContractorCategory === "All" ||
                            c.category === activeContractorCategory,
                        )
                        .map((contractor, idx) => {
                          const bidAmount =
                            selectedCost > 0
                              ? selectedCost * contractor.multiplier
                              : 25000 * contractor.multiplier;
                          const estimatedSavings =
                            selectedSavings > 0 ? selectedSavings : 5000;
                          ...
                          return (
                            <div
                              key={idx}
                              className="bg-white rounded-3xl p-6 border border-black/5 shadow-sm flex flex-col"
                            >
                              <div className="flex justify-between items-start mb-4">
                                <h4 className="text-xl font-bold text-primary pr-4">
                                  {contractor.name}
                                </h4>
                              ...
                              <div className="mb-6">
                                <span className="bg-bg px-3 py-1 rounded-lg text-sm text-primary/80 border border-black/5">
                                  {contractor.category}
                                </span>
                              </div>
                              ...
                              <div className="space-y-3 mb-6 flex-1">
                                <div className="flex items-center gap-3 text-sm text-primary/70">
                                  ...
                                  <span>
                                    DFPR License: {contractor.license}
                                  </span>
                                </div>
                                <div className="flex items-center gap-3 text-sm text-primary/70">
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-black/5">
                                  <div>
                                    ...
                                    <p className="font-medium text-primary">
                                      {formatValue(estimatedSavings)}/yr
                                    </p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-primary/50 uppercase tracking-wider mb-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white rounded-3xl p-6 border border-black/5 shadow-sm">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <MetricTooltip
                          ...
                          tooltip="The estimated total amount of money you will save each year after completing the selected upgrades."
                      ...
                      <div className="flex items-center gap-1 text-sm text-primary font-medium">
                        ...
                        {Math.round(
                          (selectedSavings / financials.totalAnnualCost) * 100,
                        )}
                        % of total cost
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl p-6 border border-black/5 shadow-sm">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <MetricTooltip
                          title="Projected Energy Usage"
                          tooltip="Your building's estimated new total energy consumption (in Megawatt-hours) after efficiency improvements."
                          titleClassName="font-medium text-primary"
                          align="center"
                        />
                      </div>
                      <div className="text-3xl font-semibold text-primary tracking-tight mb-2">
                        {Math.round(
                          (financials.electricityCost - selectedSavings * 0.7) /
                            0.095 /
                            1000,
                        )}{" "}
                        <span className="text-lg text-primary/40 font-normal">
                          MWh
                        </span>
                      </div>
                      <div className="flex items-center gap-1 text-sm text-primary font-medium">
                    ...
                    <div className="bg-white rounded-3xl p-6 border border-black/5 shadow-sm">
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <MetricTooltip
                          title="Projected ENERGY STAR"
                          tooltip="Your estimated new ENERGY STAR score after upgrades. A higher score increases building value and marketability."
                          titleClassName="font-medium text-primary"
                          align="right"
                        />
                      </div>
                      ...
                      <div className="flex items-center gap-1 text-sm text-primary font-medium">
                        ...
                        {Math.round(
                          (selectedSavings / financials.totalAnnualCost) *
                            100 *
                            0.8,
                        )}{" "}
                        points projected
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                      <div>
                        ...
                        <p className="text-sm text-primary/60">
                          Actual vs. Projected costs after retrofits
                        </p>
                      </div>
                      <div className="flex items-center gap-2">
                        <div className="bg-bg p-1 rounded-lg flex text-sm">
                          <button
                            onClick={() => setTrackingPeriod("YTD")}
                            className={cn(
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={filteredChartData}>
                          ...
                          <Line
                            type="monotone"
                            dataKey="projected"
                            name="Projected Cost (Post-Retrofit)"
                            stroke="#FF7500"
                            strokeWidth={3}
                            strokeDasharray="5 5"
                            dot={{ r: 4, strokeWidth: 2 }}
        ...
        <footer className="pt-12 border-t border-black/5 mt-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <p className="text-sm text-primary/60 leading-relaxed">
                Estimates are based on peer comparisons within the same property
                type and vintage in the Chicago area. Savings potential assumes
                upgrading to current IECC standards.
              </p>
            </div>
            <div>
              <h5 className="font-medium text-primary mb-3">Disclaimer</h5>
              <p className="text-sm text-primary/60 leading-relaxed">
                These figures are estimates for informational purposes only.
                Actual savings will vary based on specific building conditions,
                usage patterns, and final contractor pricing.
              </p>
            </div>
          </div>
          <div className="mt-8 pt-8 border-t border-black/5 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-primary/40">
            <p>
              © {new Date().getFullYear()} Civic Energy. All rights reserved.
            </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-900">
                    Energy Optimization Report.pdf
                  </h3>
              ...
              <div className="bg-white shadow-sm border border-gray-200 w-full max-w-3xl mx-auto min-h-[600px] p-10 sm:p-16 space-y-10 relative mb-20">
                ...
                <div className="border-b-2 border-primary pb-6">
                  ...
                  <p className="text-lg text-gray-600">
                    Prepared for: {building.property_name || building.address}
                  </p>
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-gray-700 leading-relaxed text-justify">
                    Based on our comprehensive analysis of your building's
                    energy consumption patterns, we have identified significant
                    opportunities for efficiency improvements. Your current
                    Energy Star Score of 72 indicates above-average performance,
                    but targeted retrofits could elevate this to 85+, resulting
                  ...
                  <div className="space-y-6">
                    <div className="border border-gray-100 rounded-xl p-6">
                      <h3 className="text-lg font-bold text-gray-900 mb-2">
                        1. HVAC Inefficiency (Off-hours)
                      </h3>
                      <p className="text-gray-700 mb-4 text-justify">
                        Analysis of smart meter data reveals significant energy
                        draw between 10 PM and 5 AM. The current BMS scheduling
                        is not properly setting back temperatures during
                        unoccupied hours.
                      </p>
                    ...
                    <div className="border border-gray-100 rounded-xl p-6">
                      <h3 className="text-lg font-bold text-gray-900 mb-2">
                        2. Lighting Retrofit Potential
                      </h3>
                      <p className="text-gray-700 mb-4 text-justify">
                        Approximately 45% of the building's lighting fixtures
                        are still utilizing legacy fluorescent tubes. Upgrading
                        to LED fixtures with occupancy sensors presents a
                ...
                <div className="max-w-sm w-full text-center space-y-5 pointer-events-auto bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm">
                      This is a premium feature. Subscribe to access detailed
                      retrofitting plans and ROI calculations.
                    </p>
                  </div>
                  ...
                  <div className="pt-2">
                    <button
                      onClick={() =>
                        navigate("/pricing", { state: { from: "/dashboard" } })
                      }
      ...
      <div className="flex flex-col lg:flex-row gap-6 justify-between">
        <div className="flex-1 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h4 className="text-xl font-semibold text-primary tracking-tight group-hover:text-primary transition-colors">
                  {title}
                </h4>
                {tooltip && (
          ...
          <div className="flex flex-wrap gap-2 pl-9">
            {Array.isArray(incentives) ? (
              incentives.map((inc, idx) => (
                <div
                  key={idx}
                  className="inline-flex items-center gap-1.5 text-xs text-blue-600 bg-blue-500/10 px-2.5 py-1 rounded-full font-medium"
                >
        ...
        <div className="flex flex-wrap lg:flex-nowrap gap-4 lg:gap-8 items-center bg-bg p-4 rounded-2xl">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-semibold text-primary tracking-tight">
              {formatValue ? formatValue(savings) : formatCurrency(savings)}
              <span className="text-sm font-normal text-primary/50">/yr</span>
            </p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-xl font-semibold text-primary tracking-tight">
              {formatValue ? formatValue(cost) : formatCurrency(cost)}
            </p>
          </div>
          <div className="w-px h-12 bg-black/5 hidden lg:block"></div>
          <div className="space-y-1">
            ...
            <p className="text-xl font-semibold text-primary tracking-tight">
              {paybackMonths}{" "}
              <span className="text-sm font-normal text-primary/50">mos</span>
            </p>
          </div>
          <div className="w-px h-12 bg-black/5 hidden lg:block"></div>
          <div className="space-y-1">
